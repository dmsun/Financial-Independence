...
"""
	The quarterly reporting is used in order to collect neighbouring months to more easily compare
...
def qtr(df_work, cwd, tableau20):
    ...
    for fin_year in unique_years:
        ...
        df_year_work = df_work.loc[(df_work.FY == fin_year), ('Date', 'Month', 'Quarter', 'Cost', 'Category')]
        df_qtr_sum = df_year_work.loc[:, ('Quarter', 'Cost', 'Category')].groupby(['Quarter','Category'], as_index = False).sum()
        unique_qtrs = list(df_year_work.Quarter.unique())
        max_qtr = df_qtr_sum.Cost.max()

        with PdfPages("{0}\\Reports\\{1} Quarterly Reports.pdf".format(cwd, fin_year)) as pdf:
            df_qtr_cat_pivot = pd.pivot_table(df_qtr_sum, values = 'Cost', index = ['Category'], columns = ['Quarter'], aggfunc = np.sum).fillna(value=0)
            fig, ax = plt.subplots()
            ...
            ax.set_title("Spending by Quarter {}".format(year))
            ...
            for qtr in unique_qtrs:
                #	select the months for the first quarter (January - March)
                df_qtr_work = df_year_work.loc[(df_work.Quarter == qtr), :]
